import re
from extraction.Landmark import ItemRule, RuleSet, IterationRule
...
#             print value['extract']
        ...
        while not rule and target_slot >= 0:
            ...
            rule_regex_string = Landmark.escape_regex_string(self._stripes[target_slot]) + rule_regex_string
            
            rule = self.__test_item_rule(rule_regex_string, Landmark.escape_regex_string(end_regex_string), pages, page_based_markup_list)
...
#                             text_between = unicodedata.normalize('NFKD', text_between).encode('ascii', 'ignore')
            ...
            for iter_stripe in sequence_pagemanager._stripes._stripes:
                if iter_rule_regex_string:
                    iter_rule_regex_string += ".*?"
                iter_rule_regex_string += Landmark.escape_regex_string(iter_stripe)
