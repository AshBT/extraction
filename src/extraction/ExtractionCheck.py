...
#     output_str = os.path.abspath(output_dir) + "/" + page_file_dir.replace('../', '').replace('/', '%2F').replace('.','') + '.html'
    
    titles = rules.names()
    titles.insert(0, 'page')
#         resultFile = open("/Users/bamana/Documents/InferLink/workspace/memex/memexpython/output/"+page_file_dir.replace('/', '%2F')+".csv",'wb')
...
#                 wr.writerow(results)
        rows.append(row)
            ...
            #This was to store the JSON outputs to send to Elastic Search
#                 json_file_str = '/Users/bamana/Documents/InferLink/workspace/memex/memexpython/output/MRS/' + file + '.json'
#                 with codecs.open(json_file_str, "w", "utf-8") as myfile:
#                     extraction['_URI'] = (subdir+"/"+file).replace(page_file_dir + "/", "")
#                     myfile.write(json.dumps(extraction, sort_keys=True, indent=2, separators=(',', ': ')))
...
def main(argv=None):
    ...
    try:
        ...
        # '/Users/bamana/Documents/InferLink/workspace/memex/memexpython/output/watable/'
